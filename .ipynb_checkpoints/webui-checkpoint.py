...
from retriever import MedicalRetriever, parse_intents_from_response
...
def load_model(cache_model):
    ...
    bert_model.load_state_dict(
        torch.load(f"model/{cache_model}.pt", map_location=device)
    )
...
def Intent_Recognition(query, llm_client):
    prompt = f"""
阅读下列提示，回答问题（问题在输入的最后）:
当你试图识别用户问题中的查询意图时，你需要仔细分析问题，并在16个预定义的查询类别中一一进行判断。对于每一个类别，思考用户的问题是否含有与该类别对应的意图。如果判断用户的问题符合某个特定类别，就将该类别加入到
输出列表中。这样的方法要求你对每一个可能的查询意图进行系统性的考虑和评估，确保没有遗漏任何一个可能的分类。
...
问题输入："{query}"
输出的时候请确保输出内容都在**查询类别**中出现过。确保输出类别个数**不要超过5个**！确保你的解释合乎逻辑！注意，如果用户询问了有关疾病的问题，一般都要先介绍一下疾病，也就是有"查询疾病简介"这个需求。
"""
    rec_result = llm_client.recognize_intent(prompt)
    print(f"意图识别结果:{rec_result}")
    return rec_result or ""


def build_knowledge_prompt(entities, intents, retrieval_result):
    prompt_blocks = []

    if "疾病症状" in entities and "疾病" not in entities:
        symptom = entities["疾病症状"]
        candidate_diseases = retrieval_result.get("candidate_diseases", [])
        if candidate_diseases:
            all_en = "、".join(candidate_diseases)
            prompt_blocks.append(
                f"<提示>用户有{symptom}的情况，知识库推测其可能是得了{all_en}。请注意这只是一个推测，你需要明确告知用户这一点。</提示>"
            )

    for item in retrieval_result.get("knowledge_blocks", []):
        prompt_blocks.append(item["prompt_block"])

    return "".join(prompt_blocks)
...
def generate_prompt(
    ...
    intents = parse_intents_from_response(response)
    yitu = intents.copy()
    ...
    if "疾病症状" in entities and "疾病" not in entities:
        candidate_diseases = retriever.infer_disease_from_symptom(
            entities["疾病症状"],
            raw_query=query,
        )
        retrieval_result["candidate_diseases"] = candidate_diseases
        if candidate_diseases:
            entities["疾病"] = candidate_diseases[0]

    if intents:
        retrieval_knowledge = retriever.retrieve_knowledge(query, entities, intents)
        retrieval_result["knowledge_blocks"] = retrieval_knowledge.get(
            "knowledge_blocks", []
        )

    prompt = (
        "<指令>你是一个医疗问答机器人，你需要根据给定的提示回答用户的问题。请注意，你的全部回答必须完全基于给定的提示，不可自由发挥。如果根据提示无法给出答案，立刻回答“根据已知信息无法回答该问题”。</指令>"
    )
    prompt += (
        "<指令>请你仅针对医疗类问题提供简洁和专业的回答。如果问题不是医疗相关的，你一定要回答“我只能回答医疗相关的问题。”，以明确告知你的回答限制。</指令>"
    )

    pre_len = len(prompt)

    prompt += build_knowledge_prompt(entities, intents, retrieval_result)

    if pre_len == len(prompt):
        prompt += (
            "<提示>提示：知识库异常，没有相关信息！请你直接回答“根据已知信息无法回答该问题”！</提示>"
        )

    prompt += f"<用户问题>{query}</用户问题>"
    prompt += (
        "<注意>现在你已经知道给定的“<提示></提示>”和“<用户问题></用户问题>”了,你要极其认真的判断提示里是否有用户问题所需的信息，如果没有相关信息，你必须直接回答“根据已知信息无法回答该问题”。</注意>"
    )
    prompt += (
        "<注意>你一定要再次检查你的回答是否完全基于“<提示></提示>”的内容，不可产生提示之外的答案！换而言之，你的任务是根据用户的问题，将“<提示></提示>”整理成有条理、有逻辑的语句。你起到的作用仅仅是整合提示的功能，你一定不可以利用自身已经存在的知识进行回答，你必须从提示中找到问题的答案！</注意>"
    )
    prompt += (
        "<注意>你必须充分的利用提示中的知识，不可将提示中的任何信息遗漏，你必须做到对提示信息的充分整合。你回答的任何一句话必须在提示中有所体现！如果根据提示无法给出答案，你必须回答“根据已知信息无法回答该问题”。</注意>"
    )
...
def main(is_admin, usname):
    ...
    if query := st.chat_input("Ask me anything!", key=f"chat_input_{active_window_index}"):
        ...
        response = Intent_Recognition(query, llm_client)
        ...
        last = ""
        stream = llm_client.generate_answer(prompt, stream=True)
        for chunk in stream:
            last += chunk
            response_placeholder.markdown(last)

        response_placeholder.markdown(last if last else "根据已知信息无法回答该问题")
        ...
        with st.chat_message("assistant"):
            st.markdown(last if last else "根据已知信息无法回答该问题")
            if show_ent:
                with st.expander("实体识别结果"):
        ...
        current_messages.append(
            {
                "role": "assistant",
                "content": last if last else "根据已知信息无法回答该问题",
                "yitu": yitu,
